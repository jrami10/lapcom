...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mt-4">
        <a href="{{ route('produits.create') }}" class="bg-blue-600 text-white px-6 py-3 rounded hover:bg-blue-700 transition duration-200">
            + Ajouter un produit
        </a>
    </div>
                    ...
                    <tbody>
                        @foreach($produits as $produit)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border px-4 py-2">{{ $produit->marque->name }}</td>
                                <td class="border px-4 py-2">{{ $produit->price }} DH</td>
                                <td class="border px-4 py-2 space-x-2">
                                    <a href="{{ route('produits.edit', $produit->id) }}" class="inline-block bg-yellow-400 text-white px-4 py-2 rounded hover:bg-yellow-500 transition duration-200">
                                        Modifier
                                    </a>
                                    <form action="{{ route('produits.destroy', $produit->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
